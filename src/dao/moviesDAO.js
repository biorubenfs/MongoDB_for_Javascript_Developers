  ...
  static async getMoviesByCountry(countries) {
    ...
    */
    ...
    try {
      ...
      cursor = await movies.find({ countries: { $in: countries } }, { id: 0, title: 1 });
      ...
      */
      ...
      const pipeline = [
        ...
        },
        {
          $lookup: {
            from: "comments",
            let: { id: "$_id" }, // This allows to use fields from the movies documents in the pipeline
            pipeline: [
              { $match: { $expr: { $eq: ["$movie_id", "$$id"] } } },  // I don't really understand the double $$ on this query
              { $sort: { date: -1 } },
            ],
            as: "comments"
          }
